Function join-object-old($first=@(), $second = $(throw “Please specify a target to join”), $where={$firstItem -eq $secondItem}, $output={$firstItem})
{
...
$Users = Import-Csv C:\users\pyoho\Downloads\Students_SAM7_export.csv
$Ous = Import-Csv C:\Users\pyoho\Downloads\Organizational_Units.csv

#join-object -first:$Users -second:$Ous -where:{Users.school_id -eq $Ous.school_id}
